...
filename = 'data/clean_corpus.txt'  #maybe_download('text8.zip', 31344016)
...
def generate_batch(batch_size, num_skips, skip_window):
    ...
    for i in range(batch_size // num_skips):
        valid_targets = [j for j in range(num_skips + 1) if j != skip_window]
        targets = np.random.choice(valid_targets, num_skips, replace=False)
...
with graph.as_default():
    ...
    # minibatch examples and all embeddings.
